...
pub struct AtomicState {
    ...
    pub journal: AbstractJournal::State,
    pub map: AbstractMap::State,
...
impl AtomicState {
    ...
    pub open spec fn map_transition(pre: Self, post: Self, req: Request, reply: Reply) -> bool
    {
        ...
        // the map, if updated, is done in a way that matches the req
        &&& MapSpec::State::next(pre.mapspec(), post.mapspec(), to_map_label(req, reply))
        // the journal is updated to stay in sync with the map
        // and we don't forget stuff at the beginning of the journal, because journal is still
        // welded to the persistent_map
        &&& post.wf()
