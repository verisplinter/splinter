...
state_machine!{ AllocationBranchBetree {
    ...
    init!{ initialize(betree: LinkedBetreeVars::State<BranchNode>) {
        ...
        let buffer_dv = betree.linked.buffer_dv;
        let (betree_likes, branch_likes) = betree.linked.transitive_likes();

        require buffer_dv.sealed_branch_roots(branch_likes.dom());

        let branch_summary = buffer_dv.build_branch_summary(branch_likes.dom());
        ...
        require addrs_closed(buffer_dv.entries.dom(), summary_aus);
    ...
    transition!{ internal_flush_memtable(lbl: Label, new_betree: LinkedBetreeVars::State<BranchNode>, branch_idx: int, new_root_addr: Address) {
        ...
        require new_betree.linked.buffer_dv.entries == pre_buffer_dv.entries.union_prefer_right(new_branch.disk_view.entries);
    ...
    transition!{ internal_grow(lbl: Label, new_betree: LinkedBetreeVars::State<BranchNode>, new_root_addr: Address) {
        ...
        require LinkedBetreeVars::State::internal_grow(pre.betree, new_betree, Internal, new_root_addr);
    ...
    transition!{ internal_split(lbl: Label, new_betree: LinkedBetreeVars::State<BranchNode>, path: Path<BranchNode>, 
        ...
        require lbl is Internal;
        require pre.is_fresh(
            to_aus(new_addrs.repr()) + seq_addrs_to_aus(path_addrs),
        );
        ...
        require LinkedBetreeVars::State::internal_split(pre.betree, new_betree, Internal, 
    ...
    transition!{ internal_flush(lbl: Label, new_betree: LinkedBetreeVars::State<BranchNode>, path: Path<BranchNode>, 
        ...
        require LinkedBetreeVars::State::internal_flush(pre.betree, new_betree, Internal, 
        ...
        let branch_deallocs = pre.branch_aus.dom() - new_branch_aus.dom() - read_ref_aus(pre.compactors);
        let new_branch_summary = pre.branch_summary.remove_keys(branch_deallocs);
        let new_summary_aus = summary_aus(new_branch_summary);

        let dealloc_branch_summary = pre.branch_summary.restrict(branch_deallocs);
    ...
    transition!{ internal_compact_abort(lbl: Label, input_idx: int, new_betree: LinkedBetreeVars::State<BranchNode>) {
        ...
        let pre_buffer_dv = pre.betree.linked.buffer_dv;
        let new_domain = restrict_domain_au(pre_buffer_dv.entries, new_summary_aus);
        let new_buffer_dv = BufferDisk{entries: pre_buffer_dv.entries.restrict(new_domain)};
    ...
    transition!{ internal_compact_complete(lbl: Label, new_betree: LinkedBetreeVars::State<BranchNode>, 
        ...
        let new_branch_summary = pre.branch_summary.insert(
            new_branch.root.au, new_branch.get_summary()).remove_keys(branch_deallocs);
        let new_summary_aus = summary_aus(new_branch_summary);
        ...
        require new_betree.linked.buffer_dv.entries == full_buffer_dv.restrict(post_buffer_domain);
    ...
    pub open spec(checked) fn inv(self) -> bool {
        ...
        // added branch inv
        &&& linked.buffer_dv.sealed_branch_roots(branch_likes.dom() + compactor_roots)
    ...
    { 
        ...
        pre_buffer_dv.build_branch_summary_insert(post_buffer_dv, pre_branch_roots, new_branch);
    ...
    fn internal_split_inductive(pre: Self, post: Self, lbl: Label, new_betree: LinkedBetreeVars::State<BranchNode>, path: Path<BranchNode>, request: SplitRequest, new_addrs: SplitAddrs, path_addrs: PathAddrs) 
    {
        ...
        let allocs = to_aus(new_addrs.repr()) + seq_addrs_to_aus(path_addrs);
        assert(allocs =~= to_aus(new_addrs.repr() + path_addrs.to_set())) by {
    ...
    { 
        ...
        restrict_domain_au_ensures(post_branch_likes, pre_buffer_dv.entries);
        ...
        pre_buffer_dv.build_branch_summary_ensures(pre_branch_roots);
        ...
        let pre_branch_dv = pre_buffer_dv.to_branch_disk();
    ...
    fn internal_compact_abort_inductive(pre: Self, post: Self, lbl: Label, input_idx: int, new_betree: LinkedBetreeVars::State<BranchNode>) 
    {
        ...
        assert(post.betree.linked.valid_buffer_dv()) by {
            ...
            post.betree.linked.buffer_likes_domain(post_betree_likes);
            restrict_domain_au_ensures(branch_likes, pre_buffer_dv.entries);
        ...
        path: Path<BranchNode>, start: nat, end: nat, input_idx: int, branch_idx: int, new_node_addr: Address, path_addrs: PathAddrs) 
    {
        ...
        pre.betree.post_compact_ensures(path, start, end, new_branch.root(), linked_new_addrs, path_addrs);
        ...
        let full_buffer_disk = pre_buffer_dv.entries.union_prefer_right(new_branch.disk_view.entries);
        let full_buffer_dv = BufferDisk{entries: full_buffer_disk};
        ...
        let pre_branch_roots =  branch_likes.dom() + pre_compactor_roots;
        let post_branch_roots = post_branch_likes.dom() + post_compactor_roots;
        let full_branch_roots = pre_branch_roots + set!{new_branch.root};

        assert(set_addrs_disjoint_aus(full_branch_roots)) by {
            assert forall |addr| pre_branch_roots.contains(addr)
            implies addr.au != new_branch.root.au
            by {
                pre_buffer_dv.sealed_branch_roots_contains(pre_branch_roots, addr);
        ...
        // assert(post.betree_aus.dom() <= pre.betree_aus.add(add_betree_aus).dom());
        let allocs = seq_addrs_to_aus(path_addrs).insert(new_node_addr.au);
        assert(allocs =~= add_betree_aus.dom()) by {
            path_addrs.to_multiset_ensures();
...
impl BufferDisk<BranchNode> {
    ...
    proof fn build_branch_summary_ensures(self, branch_roots: Set<Address>)
        requires 
            branch_roots.finite(),
            set_addrs_disjoint_aus(branch_roots),
            self.sealed_branch_roots(branch_roots),
        ensures ({
            ...
            &&& forall |au| branch_summary.contains_key(au)
                ==> #[trigger] branch_summary[au].contains(au)
            &&& forall |addr| #[trigger] branch_roots.contains(addr)
                ==> self.get_branch(addr).get_summary() <= summary_aus(branch_summary)
        })
    {
        let branch_summary = self.build_branch_summary(branch_roots);
        self.build_branch_summary_finite(branch_roots);

        assert forall |au| #[trigger] branch_summary.contains_key(au)
        ...
        implies self.get_branch(addr).get_summary() <= summary_aus(branch_summary)
        by {
            let branch = self.get_branch(addr);
            self.build_branch_summary_contains(branch_roots, addr);
            lemma_union_set_of_sets_subset(branch_summary.values(), branch.get_summary());
        }
    }

    pub proof fn build_branch_summary_insert(self, post: Self, branch_roots: Set<Address>, branch: LinkedBranch<Summary>)
        requires 
            post.to_branch_disk().wf(),
            branch.valid_sealed_branch(),
            branch.tight_disk_view_with_summary(),
            branch_roots.finite(),
            self.sealed_branch_roots(branch_roots),
            set_addrs_disjoint_aus(branch_roots + set!{branch.root}),
            self.entries.dom().disjoint(branch.disk_view.entries.dom()),
            post.entries == self.entries.union_prefer_right(branch.disk_view.entries),
            addrs_closed(self.entries.dom(), summary_aus(self.build_branch_summary(branch_roots))),
            summary_aus(self.build_branch_summary(branch_roots)).disjoint(branch.get_summary()),
        ensures
            post.sealed_branch_roots(branch_roots + set!{branch.root}),
            ...
            == self.build_branch_summary(branch_roots).insert(branch.root.au, branch.get_summary()),
    {
        ...
        if post.to_branch_disk().wf() {
            ...
            implies (#[trigger] post.get_branch(root)).valid_sealed_branch()
            by {
                let post_branch = post.get_branch(root);
                if root == branch.root {
    ...
    pub proof fn build_branch_summary_remove(self, branch_summary: Map<AU, Set<AU>>,
        ...
        requires
            ...
            addrs_closed(self.entries.dom(), summary_aus(branch_summary)),
            branch_summary == self.build_branch_summary(branch_roots),
        ensures ({
            let post_branch_summary = branch_summary.remove_keys(to_aus(branch_roots-post_branch_roots));
            let domain = restrict_domain_au(self.entries, summary_aus(post_branch_summary));
            let post_entries = self.entries.restrict(domain);
            let post_dv = BufferDisk{entries: post_entries};

            &&& post_dv.to_branch_disk().wf()
            &&& post_dv.sealed_branch_roots(post_branch_roots)
            &&& post_branch_summary == post_dv.build_branch_summary(post_branch_roots)
        ...
        })
    {
        ...
        let domain = restrict_domain_au(self.entries, post_summary_aus);

        let pre_branch_dv = self.to_branch_disk();
        ...
        implies post_dv.get_branch(root).valid_sealed_branch()
        by {
        ...
        let post_build = post_dv.build_branch_summary(post_branch_roots);
        assert(post_branch_summary == post_build) by {
            let pre_root_to_au = Map::new(|addr| branch_roots.contains(addr), |addr: Address| addr.au);
            assert(pre_root_to_au.dom() =~= branch_roots);
            let post_root_to_au = Map::new(|addr| post_branch_roots.contains(addr), |addr: Address| addr.au);
            assert(post_root_to_au.dom() =~= post_branch_roots);
